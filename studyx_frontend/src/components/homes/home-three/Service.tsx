...
const service_data: DataType[] = [
   {
      ...
      desc: "Forecast trends, customer behavior, or demand",
   },
   {
      id: 2,
      icon: "/assets/img/icon/lence.png",
      title: "Recommendation Engines & Personalization",
      desc: "Power product suggestions, content feeds",
   },
   {
      id: 3,
      icon: "/assets/img/icon/cloud.png",
      title: "Computer Vision & Image Recognition",
      desc: "Enable image classification, object detection, or visual control",
   },
   {
      id: 4,
      icon: "/assets/img/icon/server.png",
      title: "Data Engineering & AI Infrastructure",
      desc: "Connect with Slack, Trello, Google Drive & more",
   },
]

const Service = () => {
   return (
      <section className="service-section3 position-relative z-1 pt-100">
         <img src="/assets/img/element/bg-angel.png" alt="img" className="bg-angel" />
            ...
            <div className="row g-0">
               {service_data.map((item) => (
                  <div key={item.id} className="col-lg-3 col-md-6 col-sm-6">
                     <div className="service-items3 h-100 wow fadeInDown" data-wow-delay=".3s">
                        <div className="icon d-center rounded-circle w-64 h-64 mb-40">
                           <img src={item.icon} alt="img" />
                        </div>
                        <h5 className="mb-2 wow fadeInUp" data-wow-delay=".3s">
                           <Link to="/services-details" className="black-clr fw-600">
                              {item.title}
                           </Link>
                        </h5>
                        <p className="wow fadeInUp" data-wow-delay=".5s">{item.desc}</p>
                     </div>
                  </div>
               ))}
            </div>
